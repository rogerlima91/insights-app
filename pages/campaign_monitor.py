...
# ── Mock campaign data ────────────────────────────────────────────────────────
# Real Captify clients running direct deals on TTD. All figures in AUD.
RAW_CAMPAIGNS = [
    {
...
# ── Mock TTD Deal Health diagnostics (one entry per at-risk deal) ─────────────
# Mimics what the TTD Deal Health API would return for each deal ID.
DIAGNOSTICS = {
    "DL-44821": {  # Grey Goose AU — Critical 68.8%
        "health_score": 42,
        "blockers": [
            {
                "rank": 1, "impact": "HIGH",
    ...
    "DL-39847": {  # Heineken — Critical 61.8%
        "health_score": 35,
        "blockers": [
            {
                "rank": 1, "impact": "HIGH",
                "description": (
                    ...
                    "matching only 6% of available inventory — segment too narrow for required flight volume"
                ),
            },
            {
                "rank": 2, "impact": "HIGH",
                "description": (
                    "Deal floor price A$8.50 CPM exceeding TTD auto-bid A$6.20 on 74% of "
                    "ad requests — majority of inventory not clearing"
                ),
            },
            {
                "rank": 3, "impact": "LOW",
                "description": (
                    "Creative spec mismatch — 320×50 banner rejected by 58% of PG publisher "
                    "nodes (VPAID format, HTML5 expected)"
                ),
            },
        ],
        "segment_callout": {
            ...
            "issue":           (
                ...
                "~6% of Heineken's PG deal inventory — insufficient for the required delivery pace."
            ),
            "alternatives":    [
        ...
        "blockers": [
            {
                ...
                "description": (
                    ...
                    "matching 23% of available PMP inventory — moderate but improvable"
                ),
            },
            {
    ...
    "DL-77345": {  # Continental Tyres — Critical 61.3%, 6 days left
        "health_score": 28,
        "blockers": [
            {
                "rank": 1, "impact": "HIGH",
                "description": (
                    ...
                    "only 4% of available inventory — smallest addressable segment in portfolio"
                ),
            },
            {
                "rank": 2, "impact": "HIGH",
...
# ── TTD push simulator — mock PATCH response data per at-risk deal ────────────
TTD_PUSH_DATA = {
    "DL-44821": {
...
st.markdown(
    ...
    "Live pacing, deal health diagnostics and TTD optimisation tools for Captify direct deals. "
...
def pacing_bar(pacing_index, risk_color):
    ...
    # Cap the bar width at 100% — the badge communicates overpacing separately
...
st.markdown(
    ...
    "Claude reviews deal pacing as a senior programmatic specialist at Captify.</p>",
    unsafe_allow_html=True,
)
...
if st.button("✨ Run AI Analysis", type="primary"):
    if not api_key:
        ...
        # Build plain-text pacing summary to send in the prompt
        lines = []
        for c in CAMPAIGNS:
            lines.append(
                f"- {c['client']} | {c['deal_type']} | {c['deal_id']} | "
        ...
        prompt = (
            ...
            "running on The Trade Desk (TTD) for the Australian market.\n\n"
            ...
            "state the AUD underspend to date, days remaining, and consequence of inaction.\n\n"
            "**Recommended Actions by Campaign**\n"
            "For each at-risk or critical campaign, give 2–3 specific TTD actions: "
            "bid adjustments (suggest exact CPM), pacing mode changes, or targeting "
            "recommendations. Reference the deal ID and specific AUD figures.\n\n"
            "**Portfolio Insight**\n"
            "One paragraph identifying any portfolio-wide pattern (e.g. PG vs PMP "
            "performance split, deal type trends). End with one portfolio-level recommendation.\n\n"
        ...
        with st.spinner("Analysing deal pacing…"):
            response = client_ai.messages.create(
                ...
                system=(
                    ...
                    "expert in TTD deal management, PG and PMP deal structures, and "
                    "Australian programmatic advertising. Be concise, specific, "
                    "and action-oriented. Always cite deal IDs and AUD figures."
                ),
...
if "cm_ai_analysis" in st.session_state:
    # Convert **bold** markdown to HTML bold, then wrap in a styled white card
...
st.markdown(
    ...
    "mock TTD Deal Health API output.</p>",
    unsafe_allow_html=True,
)
...
for c in AT_RISK:
    ...
    with st.expander(
        f"{c['client']}  ·  {c['deal_id']}  ·  {c['risk']}  ·  "
        f"{c['days_remaining']}d remaining",
        expanded=False,
    ):
...
# ═══════════════════════════════════════════════════════════════════════════════
# SECTION 5 — TTD push simulator
# ═══════════════════════════════════════════════════════════════════════════════
st.subheader("TTD Push Simulator")
st.markdown(
    "<p style='color:#6b7280;font-size:14px;margin-top:-8px;'>"
    "Simulate a <code>PATCH /v3/deal/{id}</code> call to TTD — raises bid +25%, "
    "increases daily budget, and sets pacing to aggressive. "
    "<strong>Simulation only — no real changes are made.</strong></p>",
    unsafe_allow_html=True,
)

for c in AT_RISK:
    pd_ = TTD_PUSH_DATA[c["deal_id"]]
    ...
    with col_resp:
        if st.session_state.get(f"pushed_{c['deal_id']}"):
            # Build a mock JSON API response that looks like a real TTD PATCH reply
            budget_change_pct = (
            ...
            mock_response = {
                ...
                "estimated_recovery": {
                    ...
                    "note": (
                        "Low confidence due to compressed flight end — "
                        "monitor daily for first 48h"
            ...
            st.markdown(
                ...
                "Click the button to simulate the TTD PATCH call and see the "

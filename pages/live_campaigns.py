...
if _lc_source_mode != 'one-off':
    ...
    with col_lc_banner:
        ...
        padding:10px 16px;margin-bottom:12px;">
            <strong>📡 Live API Data Mode</strong> — showing live campaign data for all active campaigns.
        </div>
        """, unsafe_allow_html=True)
    with col_lc_refresh:
...
# ── Reference date and mock campaign data ──────────────────────────────────────────
# TODAY = last day of the 30-day performance window; matches utils/mock_data.py
TODAY = MOCK_TODAY

# Load shared mock data from utils/mock_data.py.
# MOCK_CAMPAIGNS      — list of campaign pacing dicts (same shape as old MOCK_CAMPAIGNS)
# MOCK_CAMPAIGN_BREAKDOWN — {campaign_id: {"campaigns": [{"name", "budget", "spent", "line_items"}]}}
MOCK_CAMPAIGNS, MOCK_CAMPAIGN_BREAKDOWN = get_pacing_mock_data()

# Diagnostics, push data, and performance data are not pre-populated for the new
# campaigns. The rendering code already falls back to _fallback_*() generators
# for any campaign_id not found in these dicts, so leaving them empty is safe.
DIAGNOSTICS      = {}
PUSH_DATA        = {}
PERFORMANCE_DATA = {}
...
try:
    if _alert_cfg:
        ...
        for _c in _all_campaigns:
            # campaigns already have a 'risk' field set by get_pacing_mock_data()
            _risk = _c.get("risk", "")
            if _risk == "Critical":
                _critical_count += 1
            elif _risk == "At risk":
